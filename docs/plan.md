...
- [x] watchモードのデフォルトポーリング間隔を30秒に変更
- [x] 単一マーカー方式への移行（破壊的変更）
  - `src/lib/github.ts` のマーカー関連関数を更新
  - 既存マーカー形式の後方互換性を追加（読み取りのみ）
  - マーカー関連のテストを更新
  - CLAUDE.md のマーカー説明を更新
...
**2025-10-16: start/endマーカー方式の設計上の脆弱性を発見**
- 問題: docs/plan.md内のDecision Logでマーカーについて説明している行が、実際のマーカーとして誤検出される
- 影響: pushコマンドがマーカー間の内容` / `を抽出し、リモートと一致するため更新されない結果、ローカルの変更が同期されない
- 根本原因:
  - start/endで囲む設計により、コンテンツ内にマーカー文字列があると誤検出するリスクが高い
  - 現在の実装では「コメント全体 = issync管理対象」という1:1関係なのに、範囲指定の複雑さを抱えている
- 気づき: start/endで囲む必要性が実際にはない。当初想定した「コメントの一部だけを同期」というユースケースは存在していない
...
- **理由**:
  - 実際の運用では「コメント全体 = issync管理対象」という1:1関係のみで、範囲指定の必要性がない
  - start/endで囲む設計はコンテンツ内にマーカー文字列があると誤検出する脆弱性がある
  - 先頭1行のみをチェックすることで、誤検出リスクを大幅に削減
  - よりシンプルで理解しやすい設計になる
- **実装方針**:
  - `hasIssyncMarker()`: コメント先頭行が `<!-- issync:v1 -->` で始まるかチェック
  - `addMarker()`: コメント先頭に `<!-- issync:v1 -->\n` を追加
  - `removeMarker()`: コメント先頭の `<!-- issync:v1 -->\n` を削除
  - ローカルファイルにはマーカーを含めない（リモートのみ）
- **マイグレーション**: 既存の start/end マーカー形式も読み取り可能にし、次回 push 時に新形式に自動変換
- **トレードオフ**: 破壊的変更だが、v0.x なので許容範囲。既存ユーザーは次回 push で自動的に新形式に移行

**2025-10-15: plan-template.md にissyncマーカーを含める**
- **採用**: `docs/plan-template.md` に issync マーカー（HTMLコメント形式のタグ）を含める
- **理由**:
...
- **理由**:
  - issyncの設計意図に沿う（GitHub Issueが単一真実源、ローカルは一時ファイル）
  - `.issync/`配下でgit管理外になり、意図通りの運用が自然になる
  - Issue番号ベースで複数Issue対応が直感的になる
  - `plan`という名前で、ドキュメントの用途が明確になる
- **トレードオフ**: breaking changeとなり既存ユーザーへの影響がある、デフォルト値の動的生成が必要（Issue URLパース後に決定）、ドキュメント・テストの更新が必要
- **Open Question**: Q2 を解決

**2025-10-15: watchモードのポーリング間隔デフォルト値を30秒に変更**
- **採用**: デフォルトのポーリング間隔を10秒から30秒に変更
- **理由**:
  - APIレート制限への心配を軽減（360 req/h → 120 req/h）
  - ユーザーの心理的負担を軽減（「APIを使いすぎているのでは」という不安の解消）
  - 十分な同期頻度を維持（30秒間隔でも実用上問題なし）
- **トレードオフ**: リアルタイム性が若干低下（10秒→30秒の差）、ただし`--interval`オプションで10秒に戻すことも可能
- **Open Question**: Q1 を解決
