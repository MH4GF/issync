...
- **グローバル一元管理**: 複数プロジェクトの同期を `~/.issync/state.yml` で一元管理し、一つのwatchプロセスで全体を監視する（デフォルト）
- **柔軟なスコープ選択**: プロジェクト固有の設定が必要な場合は `--local` でローカル管理も可能
...
- **メタデータ保存**:
  - **グローバル設定** (デフォルト): `~/.issync/state.yml` - 複数プロジェクト横断で一元管理
  - **ローカル設定**: `<project-root>/.issync/state.yml` - プロジェクト固有の設定
...
**グローバル設定** (`~/.issync/state.yml`, デフォルト):
- 複数プロジェクトの同期を一元管理
- `local_file` は絶対パス必須
- 一つのwatchプロセスで全てを監視可能

```yaml
syncs:
  - issue_url: https://github.com/MH4GF/issync/issues/1
    comment_id: 123456789
    local_file: /Users/mh4gf/ghq/github.com/MH4GF/issync/.issync/docs/plan-1.md
    last_synced_hash: abc123def
    last_synced_at: 2025-10-14T09:00:00Z
    poll_interval: 30
```

**ローカル設定** (`<project-root>/.issync/state.yml`, オプション):
- プロジェクト固有の設定が必要な場合に使用
- `local_file` は相対パス許可（プロジェクトルート基準）
- チーム共有が可能（リポジトリにコミット可能）
...
**設定ファイルの探索順序**:
- `--global` 指定時: `~/.issync/state.yml` のみ
- `--local` 指定時: `<project-root>/.issync/state.yml` のみ
- 指定なし（デフォルト）: `~/.issync/state.yml` を優先的に使用
...
**Q4: watch modeでstate.yml変更を即座に検知する方法は？**

- 外部から`push --force`された場合、次回ポーリングまで状態の不整合が一時的に存在する
- inotify/FSEvents等のファイル監視を使用して即座に再読み込みすべきか
- 優先度: 低（Phase 2以降で検討）

**検討案:**
- **現状維持（推奨）**: 次回ポーリングまで待機。優先度が低く、Phase 2以降で検討すれば十分
- **ファイル監視追加**: chokidarでstate.ymlを監視し、変更時に即座に再読み込み。複雑性が増すため現時点では不要
...
- check:ci（biome, eslint, type-check, knip）すべて通過
